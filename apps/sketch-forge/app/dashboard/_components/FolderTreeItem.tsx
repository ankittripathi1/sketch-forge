...
export function FolderTreeItem({
  ...
  const handleRename = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:4001/folders/${folder.id}`,
        {
          method: "PATCH",
  ...
  const handleUpdate = async (data: Partial<Folder>) => {
    try {
      const response = await fetch(
        `http://localhost:4001/folders/${folder.id}`,
        {
  ...
  return (
    <div className="relative">
      ...
      >
        <button
          ...
          className={`flex h-4 w-4 items-center justify-center rounded transition-colors hover:bg-surface-hover ${
            childFolders.length === 0 && folderPages.length === 0 ? "invisible" : ""
          }`}
        >
        ...
        <div className="opacity-0 group-hover:opacity-100 flex items-center gap-0.5 transition-opacity">
          <button
            onClick={(e) => { e.stopPropagation(); onNewPage(folder.id); }}
            title="New Page"
      ...
      {showContextMenu && (
        ...
        >
          <button
            onClick={() => { onNewPage(folder.id); setShowContextMenu(false); }}
            className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-xs text-text-body hover:bg-surface-hover"
          >
            <FilePlus size={14} /> New Page
          </button>
          <button
            onClick={() => { onNewSubfolder(folder.id); setShowContextMenu(false); }}
            className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-xs text-text-body hover:bg-surface-hover"
          >
