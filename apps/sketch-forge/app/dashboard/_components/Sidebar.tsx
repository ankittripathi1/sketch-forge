  ...
  const refreshAll = useCallback(async () => {
    try {
      ...
      if (foldersRes.ok && pagesRes.ok) {
        ...
        setPages(pagesData);
        setFolders(
          foldersData.map((f: Folder) => ({
            ...f,
            pageCount: pagesData.filter((p) => p.folderId === f.id).length,
          })),
        );
      }
    } catch (error) {
  ...
  const handleNewPage = useCallback(async (folderId: string) => {
    ...
    try {
      const res = await fetch(`${API}/pages`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title: "Untitled", elements: [], folderId }),
        credentials: "include",
      });
      if (res.ok) {
        const data = await res.json();
        refreshAll();
        router.push(`/canvas?pageId=${data.id}&folderId=${folderId}`);
      }
    } catch (error) {
  ...
  const handleDrop = async (e: React.DragEvent, targetId: string | null) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:4001"}/folders/${folderId}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!isCollapsed && pages.filter((p) => p.folderId === null).length > 0 && (
      ...
      <NewFolderDialog
        ...
        onClose={() => { setIsNewFolderOpen(false); setNewFolderParentId(null); }}
