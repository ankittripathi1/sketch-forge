...
const TOOL_GROUPS: { items: { icon: ReactNode; value: Tool; label: string }[] }[] = [
  {
    items: [
      {
...
export function Toolbar({ tool, onToolChange, onUndo, onRedo, canUndo, canRedo }: ToolbarProps) {
  return (
    ...
    <button
      ...
      className={[
        ...
        active
          ...
          : disabled
            ? "text-[oklch(0.32_0.005_260)] cursor-not-allowed opacity-50"
            : "text-[oklch(0.6_0.01_260)] hover:bg-[oklch(0.25_0.012_260)] hover:text-[oklch(0.88_0.005_260)] cursor-pointer",
      ].join(" ")}
    >
      {children}
    </button>
