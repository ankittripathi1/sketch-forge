...
export function hitTestElement(el: SketchElement, point: Point, threshold = 8): boolean {
  if (el.tool === "line") {
    return distToSegment(point, { x: el.x1, y: el.y1 }, { x: el.x2, y: el.y2 }) < threshold;
  }
  if (
    (el.tool === "freehand" || el.tool === "highlighter") &&
    el.points &&
    ...
    for (let i = 0; i < el.points.length - 1; i++) {
      if (distToSegment(point, el.points[i]!, el.points[i + 1]!) < threshold) return true;
    }
    return false;
  }
